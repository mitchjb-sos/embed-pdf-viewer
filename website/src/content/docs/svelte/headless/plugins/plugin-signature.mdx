...
| **`addEntry(entry, binaryData?)`** | Saves a new signature entry and stores its raw binary data in memory for placement. Returns the generated UUID. |
| **`removeEntry(id)`** | Deletes a signature entry. |
| **`exportEntries()`** | Returns all entries with their raw `ArrayBuffer` data, useful for saving to a backend database. |
| **`loadEntries(entries, binaryData)`** | Hydrates the plugin with previously exported entries. |
